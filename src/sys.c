...
{ 	   	  
	SCB->VTOR=NVIC_VectTab|(Offset&(unsigned int)0xFFFFFE00);//设置NVIC的向量表偏移寄存器,VTOR低9位保留,即[8:0]保留。
}
//设置NVIC分组
...
//该函数会自动开启对应中断,以及屏蔽线   	    
void Ex_NVIC_Config(unsigned char GPIOx,unsigned char BITx,unsigned char TRIM) 
{ 
...
{  
	...
	GPIOx->AFR[BITx>>3]|=(unsigned int)AFx<<((BITx&0X07)*4);
}   
//GPIO通用设置 
//GPIOx:GPIOA~GPIOI.
...
//     Fusb=336/7=48Mhz
//返回值:0,成功;1,失败。
unsigned char Sys_Clock_Set(unsigned int plln,unsigned int pllm,unsigned int pllp,unsigned int pllq)
{ 
